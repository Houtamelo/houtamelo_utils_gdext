...
impl INode for DisallowClickFocusOnParent {
    fn ready(&mut self) {
        ...
        let Some(mut parent) = base.get_parent() else {
            return godot_error!("DisallowClickFocusOnParent::ready(): Node `{}` has no parent", base.get_name())
        };
        ...
        if parent.has_signal("pressed") {
            parent.connect_flags(
                "pressed",
                &Callable::from_object_method(&self_gd, "_on_pressed"),
                ConnectFlags::DEFERRED,
            );
        } else if parent.has_signal("gui_input") {
            parent.connect_flags(
                "gui_input",
                &Callable::from_object_method(&self_gd, "_on_gui_input"),
                ConnectFlags::DEFERRED,
            );
        } else if parent.has_signal("input_event") {
            parent.connect_flags(
                "input_event",
                &Callable::from_object_method(&self_gd, "_on_input_event"),
                ConnectFlags::DEFERRED,
            );
        } else {
            godot_warn!(
                "DisallowClickFocusOnParent::ready():\nNode `{}` cannot connect to it's parent `{}`\nParent does not \
                 have any of these signals: `gui_input` | `pressed` | `input_event`",
                base.get_name(),
                parent.get_name()
            );
        }
...
impl DisallowClickFocusOnParent {
    ...
    #[func]
    fn _on_pressed(&self) { self.release_parent_focus(); }
}
